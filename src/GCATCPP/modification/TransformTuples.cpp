...
TransformTuples::TransformTuples(const std::string &from_rule, const std::string &to_rule) {
    if(from_rule.length() != to_rule.length()) {
        ...
        this-> has_error = true;
        this->rule_set = TransformTuples::transformation_by_name("I");;
        return;
    }

    this->rule_set = {from_rule, to_rule};
}
...
bool operator==(const TransformTuples &c1, const TransformTuples &c2) {
    auto set_1 = c1.rule_set;
    auto set_2 = c2.rule_set;
    if(set_1[0].length() != set_2[0].length()) {
        return false;
    }
...
TransformTuples TransformTuples::find_transformation_from_sequences(const std::string &seq_1, const std::string &seq_2) {
    ...
    for (size_t i = 0; i < seq_1.length(); ++i) {
        ...
        if(pos == std::string::npos) {
            ...
            errorManager.add_error_msg(std::string(1, from) + " has 2 mappings. [ " + alternative_to + ", " + std::string(1, to) + " ]");
            TransformTuples t_return("","");
            t_return.has_error = true;
            return t_return;
        }
