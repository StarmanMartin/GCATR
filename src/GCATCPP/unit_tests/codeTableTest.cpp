...
TEST (GenerticCodeTest, StandardCode) {
    ...
    EXPECT_EQ(ss.str(),
              "");
...
TEST (GenerticCodeTest, TheMoldProtozoanAndCoelenterateMitochondrialCodeAndTheMycoplasma_SpiroplasmaCode) {
    ...
    EXPECT_EQ(gen_codes::CodonTranslTables::getInstance().getIdxByName(
...
TEST (GenerticCodeTest, ClusterAlgorthemCusterSetWeights_Stable) {
    ...
    cca.reset_all_weights();

    condS4 = cca.get_conductance_of_cluster(S4);
    EXPECT_NEAR(0.666, condS4, 0.001);
}
...
TEST (GenerticCodeTest, ClusterAlgorthem) {
    ...
    double average = cca.get_average_conductance();
    double weighted_average = cca.get_weighted_average_conductance();
    double max = cca.get_max_conductance();
    double min = cca.get_min_conductance();

    double weighted_average_res = (64.0/3.0 + 16.0 + 7.0/3.0 + 0.740741 * 6 + 0.851852 * 3.0 + 2.0) / 64.0;

    EXPECT_NEAR(1, min, 0.0001);
    EXPECT_NEAR(0.666667, max, 0.0001);
    EXPECT_NEAR(0.811287, average, 0.0001);
    EXPECT_NEAR(weighted_average_res, weighted_average, 0.0001);
}
