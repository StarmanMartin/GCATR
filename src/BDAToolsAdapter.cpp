...
#include "GCATCPP/codes/SequenceFactory.h"
...
//' @export
// [[Rcpp::export]]
bool seq_start_bda(StringVector code, int length = 3) {
  auto code_vec = RAdapterUtils::as_cpp_string_vector(code);
  auto a = SequenceFactory::rFactor(code_vec, length);
  return _strat_bda(a);
}
...
//' @export
// [[Rcpp::export]]
bool code_start_bda(StringVector code, int length = -1) {
  auto code_vec = RAdapterUtils::as_cpp_string_vector(code);
...
List run_bda_for_all_rna_codons() {
  ...
  return Rcpp::List::create(Rcpp::Named("code") = BDATools::get_all_rna_codons(),
                              Rcpp::Named("bda") = return_val);
}