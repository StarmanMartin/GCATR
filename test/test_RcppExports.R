...
# 9. non 2-circular, string split by ' '  
test_that('is not 2-circular', {
  expect_false(code_check_if_k_circular(2,'UGG CUG GGC UGU'))
})

# 10.  2-circular, string split by ' '
test_that('is not 2-circular', {
  expect_true(code_check_if_k_circular(2,'ACG GUA CGU CGG UAC',3))
})

# 11. non 2-circular, string vector  
test_that('is not 2-circular', {
  expect_false(code_check_if_k_circular(2,c('UGG','CUG','GGC','UGU')))
})

# 12.  2-circular, string vector'
test_that('is not 2-circular', {
  expect_true(code_check_if_k_circular(2,c('ACG','GUA','CGU','CGG','UAC')))
})

# 13. 1-circular,  string vector
test_that('is not 1-circular', {
  expect_true(code_check_if_k_circular(1,c('UGG','CUG','GGC','UGU')))
})

# 14. non 2-circular, string with no delimiter, tuple length 3
test_that('is not 2-circular', {
  expect_false(code_check_if_k_circular(2,'UGGCUGGGCUGU',3))
})

# 15.  2-circular, string with no delimiter, tuple length 3
test_that('is not 2-circular', {
  expect_true(code_check_if_k_circular(2,'ACGGUACGUCGGUAC',3))
})

# 16. non 2-circular, string split by ' '  
test_that('is not 2-circular', {
  expect_false(code_check_if_k_circular(2,'UGG CUG GGC UGU'))
})

# 17.  2-circular, string split by ' '
test_that('is not 2-circular', {
  expect_true(code_check_if_k_circular(2,'ACG GUA CGU CGG UAC',3))
})

# 18. non 2-circular, string vector  
test_that('is not 2-circular', {
  expect_false(code_check_if_k_circular(2,c('UGG','CUG','GGC','UGU')))
})
