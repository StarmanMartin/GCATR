#' @title BDA (binary dichotomic algorithm) Object
#' 
#' @description  This Object contains a binary dichotomic algorithm (BDA) procedure. Use \link{start_bda} or \link{code_start_bda} to get a BDATools object. \cr 
...
#' @title Start a BDA procedure for all codons
#' 
#' @description  This Function starts a binary dichotomic algorithm (BDA) procedure for all codons (see \link{get_rna_codon_list}).
...
#' @title Start a BDA procedure for a code.
#' 
#' @description Each added binary dichotomic algorithm (BDA) results one binary digit for each codon.
#' You can add multiple BDAs for one procedure. To add a rule, you need to start a BDA procedure first.
#' To do so you can either call \link{code_start_bda} or to start a procedure
#' for all RNA codons call \link{start_bda}.
#' To add a BDA simply Call \link{add_rule}. To execute the BDAs call \link{run_bda}.
...
#' @title Add a binary dichotomic algorithm (BDA).
#' 
#' @description Each added binary dichotomic algorithm (BDA) results one binary digit for each codon.
#' You can add multiple BDAs for one procedure. To add a rule, you need to start a BDA procedure first.
#' To do so you can either call \link{code_start_bda} or to start a procedure
...
#' @title Execute BDA procedure.
#' 
#' @description This function executes the currently open binary dichotomic algorithm (BDA) procedure. Therefore, you need to start a BDA procedure.
...
#' @title Execute BDA procedure (only works for all codons).
#' 
#' @description This Function executes a binary dichotomic algorithm (BDA) procedure. It only works if the BDA procedure has been started by the \link{start_bda} function.
#' Each added binary BDA results one binary digit for each codon.
#' You can add multiple BDAs for one procedure. To add a rule, you need to start a BDA procedure first.
#' To do so you can either call \link{code_start_bda} or to start a procedure
#' for all RNA codons call \link{start_bda}.
